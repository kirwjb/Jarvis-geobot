...
import { go, bindNavigation } from './core/router.js';
...
function handleClick(event){
 ...
 if(action==='language'){event.preventDefault();event.stopPropagation();toggleLanguage();return}
...
document.addEventListener('input',event=>{
 if(event.target.id==='poi-search')searchPlaces(event.target.value);
 if(event.target.id==='region-search')filterRegions(event.target.value);
 if(event.target.id==='city-search')filterCities(event.target.value);
});
...
async function init(){
 ...
 if(state.region){try{state.cities=await import('./core/api.js').then(({request})=>request(`/cities/${encodeURIComponent(state.region)}`)||[]);}catch(_) {}}
 persist();
}
