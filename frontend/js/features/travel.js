...
export function renderCities(list=state.cities){const grid=$('#cities-grid');if(!grid)return;grid.innerHTML=list.length?list.map((c,i)=>`<button class="card ${state.city===c.name?'active':''}" type="button" data-action="city" data-index="${i}"><span>${esc(c.name)}</span></button>`).join(''):`<div class="empty">${esc(t('nothing'))}</div>`;}
...
export function pickCity(index){
 const city=state.cities[index];if(!city)return;state.city=city.name;
